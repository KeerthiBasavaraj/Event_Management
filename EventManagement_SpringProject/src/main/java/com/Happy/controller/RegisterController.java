...
public class RegisterController {
  ...
  //data lost ll not be happen --> ModelAttribute
    public String userRegister(@ModelAttribute RegisterEntity user, Model model) {
        ...
        if (exist == false) {
            ...
            if (uid > 0) {
            	//model is used to send the message
            	//model cannot be used in whole project
    ...
    //Get the data from the url --> @RequestParam
    public ResponseEntity<Boolean> checkCode(@RequestParam String useremail) {
        ...
        return ResponseEntity.ok(isCodeAvailable); //it will return the data with http status code.
    }
